...
for country in cfg.COUNTRIES:
    ...
    for C in cfg.SVR_PARAMS['C']:
        for epsilon in cfg.SVR_PARAMS['epsilon']:
            ...
            print(f"{country} | C={C}, epsilon={epsilon} -> RMSE: {rmse:.3f}, MAE: {mae:.3f}, MAPE: {mape_val:.2f}%")

            forecast_df = pd.DataFrame({'y_true': y_test, 'y_pred': y_pred}, index=X_test.index)
            forecast_df.to_csv(os.path.join(output_path, f"{country}_svr_C{C}_eps{epsilon}.csv"))
